...
class TreeCompiler(Bundle):
	...
	def __init__(self, *args, **kwargs):
		...
		super(TreeCompiler, self).__init__(*args, **kwargs)

		# Determine compiler path (real path, with resolved symlinks)
		compiler_name = self.cfg['name'].lower()
		installdir = self.cfg['sanity_check_paths']['dirs'][0]

		path_to_compiler = which(os.path.join(installdir, 'bin', compiler_name))
		if path_to_compiler:
	...
	def make_module_step(self, fake=False):
		"""
		Custom module step for TreeCompiler: make 'EBROOT' and 'EBVERSION' reflect actual system compiler version
		and install path.
		"""
		...
		# Generate module
		res = super(TreeCompiler, self).make_module_step(fake=fake)
	...
	def make_module_extend_modpath(self):
		...
		res = super(TreeCompiler, self).make_module_extend_modpath()
